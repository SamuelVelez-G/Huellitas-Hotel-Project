...
const HOSPEDAJE = {
  perro: {
    luxury: {
      name: "Habitación Luxury",
      description: "Privada premium",
      price: 100000,
    },
    confort: {
      name: "Habitación Confort",
      description: "Semi privada",
      price: 80000,
    },
    familiar: {
      name: "Suite Familiar",
  ...
  aves: {
    ...
    confort: {
      name: "Aviario Confort",
      description: "Semi privada",
      price: 80000,
    },
    familiar: {
      name: "Aviario Familiar",
      description: "Familiar",
      price: 170000,
    },
  },
  ...
  pequenos: {
    luxury: {
      name: "Pequeños Huéspedes Luxury",
      description: "Privada premium",
      price: 100000,
    },
    confort: {
      name: "Pequeños Huéspedes Confort",
      description: "Semi privada",
      price: 80000,
    },
    familiar: {
      name: "Pequeños Huéspedes Explorador",
      description: "Familiar",
      price: 170000,
    },
  },
...
function updateHospedajeDates(index, field, value) {
  ...
  document.getElementById(`subtotal-${index}`).innerText =
    currencyFormatter.format(petsData[index].subtotal);
  renderSummary();
}
...
// Incrementa o decrementa la cantidad de un servicio adicional.
function incrementExtraService(index, serviceKey, delta) {
  const pet = petsData[index];
  const currentQty = pet.extraServices[serviceKey] || 0;
  const newQty = currentQty + delta;

  if (newQty <= 0) {
    delete pet.extraServices[serviceKey];
  } else {
    pet.extraServices[serviceKey] = Math.min(newQty, maxServiceQty);
  }

  pet.subtotal = calculateSubtotal(pet);
  saveToLocalStorage();
  renderQtyControl(index, serviceKey);
  document.getElementById(`subtotal-${index}`).innerText =
    currencyFormatter.format(pet.subtotal);
  renderSummary();
}

// Elimina un servicio adicional del carrito
function removeExtraService(index, serviceKey) {
  const pet = petsData[index];
  if (pet.extraServices[serviceKey]) {
    delete pet.extraServices[serviceKey];
    pet.subtotal = calculateSubtotal(pet);
    saveToLocalStorage();
    renderQtyControl(index, serviceKey);
...
function renderQtyControl(index, serviceKey) {
  const controlEl = document.getElementById(
    `qty-control-${index}-${serviceKey}`,
  );
  if (controlEl) {
    controlEl.outerHTML = buildQtyControlHtml(index, serviceKey);
  }
}

function buildQtyControlHtml(index, serviceKey) {
  const pet = petsData[index];
  const isMainService = pet.service === serviceKey;
  const qty = pet.extraServices[serviceKey] || 0;

  if (isMainService) {
    return `<div id="qty-control-${index}-${serviceKey}"></div>`;
  }

  if (qty === 0) {
    return `
      <div id="qty-control-${index}-${serviceKey}">
        <button type="button" class="btn btn-sm btn-outline-secondary-custom" onclick="incrementExtraService(${index}, '${serviceKey}', 1)">
          <span class="material-symbols-outlined fs-6 align-middle me-1">add</span>Agregar
        </button>
      </div>
    `;
  }
  ...
  return `
    <div id="qty-control-${index}-${serviceKey}" class="d-flex align-items-center gap-2">
      <div class="d-flex align-items-center bg-light rounded-pill border">
        <button type="button" class="btn btn-sm btn-light rounded-circle p-1 d-flex" onclick="incrementExtraService(${index}, '${serviceKey}', -1)" aria-label="Disminuir cantidad">
          <span class="material-symbols-outlined fs-6">remove</span>
        </button>
        <span class="px-2 fw-semibold">${qty}</span>
        <button type="button" class="btn btn-sm btn-light rounded-circle p-1 d-flex" onclick="incrementExtraService(${index}, '${serviceKey}', 1)" aria-label="Aumentar cantidad" ${qty >= maxServiceQty ? "disabled" : ""}>
          <span class="material-symbols-outlined fs-6">add</span>
        </button>
      </div>
      <button type="button" class="btn btn-sm btn-outline-danger d-flex align-items-center p-1" onclick="removeExtraService(${index}, '${serviceKey}')" title="Eliminar del carrito">
...
function renderSelectHospedaje(pet) {
  ...
  return Object.keys(habitacionesDisponibles)
    .map((roomKey) => {
